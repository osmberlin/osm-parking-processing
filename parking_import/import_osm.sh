...
function import_spatialfilter () {
  ...
  do
    ...
    then
      ...
      ogr2ogr \
        ...
        -sql "SELECT '${LAYERNAME}' AS name, name as label FROM ${LAYERNAME}" \
  ...
  echo "cleanup table spatialfilter done"
}
...
while : ; do
  ...
  PG_SCHEMA_MIDDLE_EXISTS=$(psql -XAt -c "SELECT EXISTS (SELECT FROM information_schema.schemata WHERE schema_name = '${PG_SCHEMA_MIDDLE}');")
  if [ "${PG_SCHEMA_MIDDLE_EXISTS}" = "t" ] ; then
    echo "current replication status: "
    osm2pgsql-replication status --middle-schema="${PG_SCHEMA_MIDDLE}"
  fi
